...
router.post('/register',async(req,res)=>{
    ...
    const userExist = await User.findOne({email});
        ...
        if(password != cpassword){
           return  res.status(422).json({message : "Password is not match"})
        }
...
router.post('/profile',async(req,res) => {
    ...
    if(!userExist){
     ...
     console.log('Sucess');
    return res.status(200).json({ message: 'user contact sucessfully' });
});
   ...
   // console.log(password);
    ...
    try{
     ...
     //   console.log(token);
        ...
        if(isMatch){
            return res.status(201).json({message : "You are login ",id : userExist._id})
        }else{
            return res.status(422).json({Error : "Invalid credentials"})
        }
